...
- **콘텐츠 섹션** — 본문(제한 마크다운)·제목(강조색 배경 옵션)·이미지(업로드 시 WebP 자동 변환 후 R2 저장, 등급별 용량·일일 개수 제한)·유튜브(nocookie)·버튼/링크·구분선·임베드(HTML)
- **초안 → 게시 모델** — 편집은 초안으로 누적, ‘게시’ 시 완성 HTML 스냅샷을 만들어 공개 (공개 요청은 KV/D1 스냅샷 1왕복)
- **테마** — 색상 프리셋 5종 + 개별 색상·한글 구글폰트 프리셋·헤더/내비 위치(상단·좌측)
- **관리** — 요약 카드·표(생성일·최종수정·클릭수)·QR·조회수 집계
