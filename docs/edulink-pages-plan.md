...
## 13. 사용성 및 디자인 고도화 (2026-07-06)

사용자 편의성 극대화 및 미려한 디자인 제공을 위해 페이지 대시보드와 편집기 영역 전반의 기능 고도화를 단행했다.

### 13.1 페이지 대시보드 UI/UX 고도화
- **대시보드 레이아웃 개편**: 페이지 관리 화면을 기존 테이블 레이아웃에서 단축주소와 일치하는 형태의 미려한 카드/그리드로 개편.
- **드로워 연동**: 조회수 접속통계(차트) 및 QR 코드 관리 기능을 모달 대신 드로워(Drawer) 형태로 통일하고 복사 시 토스트 피드백 제공.
- **게시 건수 버그 수정**: 빈 경로 스냅샷 중복 집계를 제외하여 실제 디자인한 페이지 수만큼만 성공 건수로 노출되도록 수정.

### 13.2 편집기 및 디자인 설정 고도화
- **작업 공간 내 디자인 설정 통합**: 디자인 설정을 팝업 모달 대신 편집기 중앙 작업 영역에 꽉 차게 렌더링하여 일체화된 조작 경험 제공.
- **글꼴 지정 셀렉트바 형식 개편**: 수많은 글꼴 프리셋 버튼 그룹을 1개의 카테고리별 드롭다운 셀렉트 박스(`optgroup`)로 단순화하여 화면 활용도 개선.
- **사용자 구글 폰트 히스토리**: 직접 입력한 구글 폰트를 브라우저 로컬 스토리지(`localStorage`)에 저장하고 글꼴 메뉴에 "사용자 지정 글꼴"로 유지.
- **헤더 설정 행 배치**: 헤더 제목 입력 인풋과 '헤더 표시' 여부 체크박스를 같은 행(Row)에 가로 배치하여 여백을 최적화.
- **경고창 다이얼로그 추가**: 디자인을 변경했으나 저장하지 않고 닫으려 할 때 미저장 안내 경고 팝업 제공.

### 13.3 컴포넌트 편의성 및 신규 기능
- **본문 URL 자동 링크 인식**: 본문 텍스트 내 raw URL을 파싱하여 타겟 새창(`target="_blank"`) 형태의 클릭 가능한 앵커 태그로 렌더링.
- **링크 버튼 정렬 선택**: 링크 섹션 내부의 버튼을 "좌측 / 중앙 / 우측"으로 정렬할 수 있는 설정 추가 및 CSS 연동.
- **방문자 카운터 표시**: 디자인 설정에서 "방문자 수(클릭 수) 카운터 표시" 기능을 키고 켜는 옵션을 추가. 메뉴 위치가 상단인 경우 헤더 맨 우측에, 좌우 메뉴인 경우 하단에 방문자 수가 세련되게 정렬되어 렌더링됨.
- **깃북 스타일 하단 네비게이션**: 깃북(GitBook) 형태의 하단 이전/다음 이동 카드 추가(이전 페이지 명칭 우정렬 + 다음 페이지 명칭 좌정렬).

---
*v2.1 — 2026-07-06 사용성 및 디자인 고도화(Step 18 및 추가 개선사항 피드백 반영) 내역 추가.*
